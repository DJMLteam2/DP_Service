...
public class ViewController {
    ...
    public String detail(@PathVariable Long id, Model model) {
        ...
        if (travelDto != null) {
            ...
            // travelDto가 null일 때 처리 (예: 에러 메시지 추가)
            model.addAttribute("errorMessage", "없습니다.");
        }
    ...
    // @PostMapping("/create")
    // public String answer(QuestionForm questionForm, RedirectAttributes
    // redirectAttributes) {

    // if (questionForm.getQuestion().isEmpty()){
    // redirectAttributes.addFlashAttribute("errorMessage", "빈칸입니다! 하고싶은 여행을
    // 작성해주세요!");

    // return "redirect:/";
    // }

    // List<FastAPIAnswerDTO> fastAPIAnswerDTOs =
    // searchService.searchViewController(questionForm, null);
    // // Flash 속성 추가
    // redirectAttributes.addFlashAttribute("searchResults", fastAPIAnswerDTOs);
    // redirectAttributes.addFlashAttribute("questionForm", questionForm);
    ...
    // @PostMapping("/mainImg")
    // public String imgAnswer(@RequestParam("imageValue") String imageText,
    // RedirectAttributes redirectAttributes) {
